...
public class CommentaryDAO {
    ...
    public Commentary getCommentaryById(int id) throws SQLException, ParseException {
        PreparedStatement ps = connection.prepareStatement("select * from commentaries where id = ?");
        ps.setInt(1, id);
        ResultSet rs = ps.executeQuery();
        Commentary e = null;
        while (rs.next()) {
            e = new Commentary(rs.getInt("id"), rs.getInt("user_id"),
    ...
    public List<Commentary> getArticleCommentaries(int id, String type) throws SQLException, ParseException {
